...
export async function POST(req: Request) {
  try {
    ...
    for (const scene of scenes) {
      ...
      if (!prompt) continue;
      const img = await generateImage({ model: record.imageModel, prompt: withRequestNonce(prompt, force) });
    ...
    if (!coverFile && record.outline) {
      ...
      const img = await generateImage({ model: record.imageModel, prompt: withRequestNonce(coverPrompt, force) });
      const buffer = Buffer.from(img.data, "base64");
      coverFile = "cover.png";
