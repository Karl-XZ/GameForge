...
Regeneration is treated as a first-class feature. When you click **Single Asset Retry** or re-run **Generate All**, requests are forced fresh (unique request stamps + cache-busting) so you don’t get stuck on the same image.
...
* **Schema validation** for JSON outputs (so steps don’t drift)
